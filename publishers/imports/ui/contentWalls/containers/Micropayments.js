...
import Micropayments from '../components/Micropayments.jsx';
...
function composer(props, onData) {
  ...
  const totalEarned = product.paygIncome || 0;
  ...
  if (totalEarnedToday === null) {
    ...
    Meteor.call('contentWall/dailyStats', product._id, { beginDate, endDate }, (err, data) => {
      if (err) {
  ...
  onData(null, {
    ...
    changePeriod, isVerified,
  });

  Meteor.call('contentWall/dailyStats', product._id, datePeriod, (err, data) => {
    if (err) {
      console.log(err); // eslint-disable-line no-console
      return;
    }
    ...
    onData(null, {
      ...
      totalEarnedToday, period: periodVar.get(),
      changePeriod, chartData, isVerified,
    });
  });
...
export default composeWithTracker(composer)(Micropayments);
